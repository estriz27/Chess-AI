...
def findBestMoveAI():
    ...
    possible_moves = chessgame.get_moves('w')
...
def runGame():
    ...
    while chessgame.status !=2 or chessgame.status != 3:
        ...
        AImoveValue = findBestMoveAI()
        AImove = findMoveBasedonValue(AImoveValue,'w')
        AIPoints += AImoveValue
        

 #       AImove = findMoveBasedonValue(movevalue, 'w')
        ...
        blackPoints += findMoveValue(move)
        print("minimax " + str(blackPoints))
